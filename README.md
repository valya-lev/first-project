...
```echo "текст" >> file.txt``` - добавить информцию в существующий файл
...
```git branch``` - просмотреть ветки проекта (показывает только локальные ветки – те, которые существуют на вашем локальном компьютере)    
### Создаём ветку  
...
### Шагаем с ветки на ветку и осматриваемся
```git checkout <название_ветки>``` - переключиться на другую ветку
```git checkout -b <название_ветки>``` - создать ветку и сразу переключиться на неё
```git branch -a``` - просматриваем все ветки

## Сравниваем ветки  
```git diff ветка1 ветка2``` - сравнение веток по их названиям, например: ```git diff main feature/my-feature``` - такая команда выведет разницу между основной веткой и веткой ```feature/my-feature```  
вместо ```ветка1(2)``` можно указывать: реальные названия веток (```feature/my-feature```), хэш номер ветки(```2ea56ab```), синоним ветки (```master, head```)  
...
## Делаем pull request  
**Первый способ**:  
При создании новой ветки в удалённом репозитории Git распечатает сообщение. Оно включает ссылку на создание пул-реквеста. Останется только скопировать её в адресную строку браузера, заполнить необходимые поля и нажать **Create pull request**. Многие терминалы также позволяют кликнуть на эту ссылку — напрямую или через комбинацию ```Cmd / Ctrl``` + клик.  
...
- Нажмите на зелёную кнопку **New pull request**
- Выберите названия веток: ветка «откуда» (из которой будет происходить пул-реквест) и ветка «куда» (в которую он будет осуществлён).
...
Алгоритм такой. Сначала нужно перейти в локальный репозиторий и убедиться, что вы находитесь в правильной ветке, — как правило, это основная ветка ```main``` (или ```master```). Затем можно ввести команду - ```git pull```/   
