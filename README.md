...
```git reset --hard <commit hash>``` - иногда нужно «откатить» то, что уже было закоммичено, то есть вернуть состояние репозитория к более раннему - по сути удаляет эти изменения  
